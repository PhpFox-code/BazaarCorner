		...
		<div class="p_10{if $phpfox.iteration.articles/2} row1{else} row2{/if}{if $phpfox.iteration.articles==1} row_first{/if}">
			...
			{if $aArticle.user_id==Phpfox::getUserId()}
				<div class="p_top_8 t_right">
					{if $iATO > 0 && $iATO != $aArticle.article_id}<a href="{url link='article.addsection' ai=$iATO ins=$aArticle.article_id}">Insert</a> | {/if}<a href="{url link='article.add' id=$aArticle.article_id}">Edit Article</a> | 
					{if $aArticle.is_featured==0}<a href="{url link='article.feature' id=$aArticle.article_id}" >Feature</a>{else}<a href="{url link='article.unfeature' id=$aArticle.article_id}" >Unfeature</a>{/if} | <a href="{url link='article.delete' id=$aArticle.article_id}" class="sJsConfirm">Delete</a>
				</div>
			{/if}
