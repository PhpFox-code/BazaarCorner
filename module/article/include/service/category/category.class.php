...
class Article_Service_Category_Category extends Phpfox_Service 
{
	private $_sOutput = '';
	...
	{		
		$sCacheId = $this->cache()->set('article_category_browse' . ($sCategory === null ? '' : '_' . md5($sCategory)));
		...
		{		
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				if ($sCategory === null)
				{				
					$aCategories[$iKey]['url'] = Phpfox::getLib('url')->makeUrl('article.category', $aCategory['name_url']);
				}
				else 
				{
					...
					$aCategories[$iKey]['url'] = Phpfox::getLib('url')->makeUrl('article.category', $aParentCache);
				}
				...
				//if ($sCategory === null)
				{
					...
					{
						$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::getLib('url')->makeUrl('article.category', array($aCategory['name_url'], $aSubCategory['name_url']));
					}
				}
			}
	...
	public function get()
	{
		$sCacheId = $this->cache()->set('article_category_display_' . $this->_sDisplay . '_' . Phpfox::getLib('locale')->getLangId());
	...
	{		
		$sCacheId = $this->cache()->set('article_parent_breadcrumb_' . md5($sCategory));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		if ($aCategories === null)
		{
			$aCategories = $this->database()->select('pc.parent_id, pc.category_id, pc.name, pc.name_url')
				->from(Phpfox::getT('article_category_data'), 'pcd')
				->join($this->_sTable, 'pc', 'pc.category_id = pcd.category_id')
				->where('pcd.article_id = ' . (int) $iId)
		...
		{			
			...
			{				
				if ($aCategory['parent_id'] > 0)
				{
					...
					$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::getLib('url')->makeUrl('article.category', $aCache));
				}				
				else 
				{
					$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::getLib('url')->makeUrl('article.category', $aCategory['name_url']));
				}				
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::getLib('url')->makeUrl('article.category', $aCategories[0]['name_url']));
		}
	...
	public function getCategoryIds($iId)
	{
		$aCategories = $this->database()->select('category_id')
			->from(Phpfox::getT('article_category_data'))
	...
	{
		$sCacheId = $this->cache()->set('article_category_childern_' . $sCategory);
	...
	{
		$sCacheId = $this->cache()->set('article_category_parent_' . $sCategory);
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('article.service_category_category__call'))
		{
			return eval($sPlugin);
		}
	...
	private function _getParentsUrl($iParentId, $bPassName = false)
	{
		...
		if ($iCnt === 1)
		{
			...
			$sCacheId = $this->cache()->set('article_category_url' . ($bPassName ? '_name' : '') . '_' . $iParentId);
