...
<ul>
        <li>
                <a href="#" class="has_drop_down"><span>{phrase var='pages.account'}</span></a>
                <ul>
...
{foreach from=$aRightMenus key=iKey item=aMenu}
        <li><a href="{url link=$aMenu.url}"{if isset($aMenu.children) && count($aMenu.children) && is_array($aMenu.children)} class="has_drop_down no_ajax_link"{/if}><span>{phrase var=$aMenu.module'.'$aMenu.var_name}{if isset($aMenu.suffix)}{$aMenu.suffix}{/if}</span> </a>					
