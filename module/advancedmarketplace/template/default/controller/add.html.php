...
<form method="post" action="{url link='current'}" enctype="multipart/form-data" onsubmit="return startProcess({$sGetJsForm}, false);" id="js_advancedmarketplace_form">
	...
	<div id="js_mp_block_detail" class="js_mp_block page_section_menu_holder">
		...
		<div class="table">
			...
			<div class="table_right">
				{*<textarea style="width: 970px;" rows="6" name="val[short_description]" >{if isset($aForms.short_description)}{$aForms.short_description}{/if}</textarea>*}
				<textarea style="width: 970px;" rows="6" name="val[short_description]" >{value type='textarea' id='short_description'}</textarea>
			</div>
		</div>
		<div class="table">
			<div class="table_left">
		...
		<div class="table_clear">
			...
			{if !$bIsEdit}<input type="submit" name="val[draft]" value="{phrase var='blog.save_as_draft'}" class="button button_off" />{/if}
			{if $bIsEdit && $aForms.post_status == 2}
				<input type="submit" name="val[draft_publish]" value="{phrase var='blog.publish'}" class="button button_off" />
			{/if}
		</div>
	</div>
