	...
	<li>
		<div class="block_listing_image" style="width: 65px;">
			<a href="{permalink module='advancedmarketplace.detail' id=$aMiniListing.listing_id title=$aMiniListing.title}">
				{*img server_id=$aMiniListing.server_id title=$aMiniListing.title path='advancedmarketplace.url_image' file=$aMiniListing.image_path suffix='_50_square' max_width='50' max_height='50'*}
				{if $aMiniListing.image_path != NULL}{*
					*}<img title="{$aMiniListing.title}" src="<?php echo $this->_aVars['advancedmarketplace_url_image'] . PHPFOX::getService("advancedmarketplace")->proccessImageName($this->_aVars["aMiniListing"]["image_path"], "_120"); ?>" max-width='60' max-height='60' style="max-height: 60px; max-width: 60px; margin-right: 5px;" />{*
				*}{else}{*
