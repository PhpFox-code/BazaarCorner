...
{if $aListing.image_path != NULL}
	<div class="slideblock" style="width: 520px;">
		<div style="width: 520px; height: 322px;" class="bigimg">
			{if $aListing.image_path != NULL}{*
				*}<img title="{$aListing.title}" src="{$advancedmarketplace_url_image}<?php echo PHPFOX::getService("advancedmarketplace")->proccessImageName($this->_aVars["aListing"]["image_path"], ""); ?>" style="max-width:520px;max-height:322px;" />{*
			*}{else}{*
			...
			<div class="window">
				<div class="slider">
					...
					{foreach from=$aImages name=images item=aImage}
						<img style="visibility: hidden;" class="buff" src="{$advancedmarketplace_url_image}<?php echo PHPFOX::getService("advancedmarketplace")->proccessImageName($this->_aVars["aImage"]["image_path"], ""); ?>" ?>
					{/foreach}
						<ul>
							{foreach from=$aImages name=images item=aImage}
								<li>
								<a ref="{$advancedmarketplace_url_image}<?php echo PHPFOX::getService("advancedmarketplace")->proccessImageName($this->_aVars["aImage"]["image_path"], ""); ?>" class="slideclick js_marketplace_click_image no_ajax_link" href="{*
