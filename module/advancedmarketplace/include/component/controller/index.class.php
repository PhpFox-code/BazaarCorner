...
class advancedmarketplace_Component_Controller_Index extends Phpfox_Component {
    ...
    public function process() {
        ...
        if (!defined('PHPFOX_IS_USER_PROFILE') && $this->request()->get('view') == '' && !$this->request()->getInt('delete'))
        {                    
            $this->template()->setTitle(Phpfox::getPhrase('advancedmarketplace.advanced_advancedmarketplace'));
		...
		{
			phpfox::getLib('url')->send('advancedmarketplace.search.when_'.Phpfox::getLib('database')->escape($sWhen));
		}
        ...
        $sCategory = $this->search()->get('category');
        $sText = $this->search()->get('text');
        $this->setParam('sCategory', $sCategory);
        $this->setParam('sText', $sText);
