...
class AdvancedMarketplace_Component_Controller_Add extends Phpfox_Component
{
	...
	public function process()
	{
		...
		if ($aVals = $this->request()->get('val'))
		{
			...
			// {phrase var='advancedmarketplace.afield_is_required'}
		...
		{
			$aMenus = array(
				'detail' => Phpfox::getPhrase('advancedmarketplace.listing_details'),
				'customize' => Phpfox::getPhrase('advancedmarketplace.photos'),
				'invite' => Phpfox::getPhrase('advancedmarketplace.invite')
			);
						...
						$oSWF_settings =
						{
							...
							get_settings: function()
							{
								...
								swfu.customSettings.sAjaxCallAction = function(){
									...
									/* $("#js_advancedmarketplace_form").submit(); */
									 window.location = ' . $sInviteLink . ';
								}
		...
		$this->template()->setTitle(($bIsEdit ? Phpfox::getPhrase('advancedmarketplace.editing_listing') . ': ' . $aListing['title'] : Phpfox::getPhrase('advancedmarketplace.create_a_advancedmarketplace_listing')))
			...
			->setHeader(array(
					...
					// '<script type="text/javascript">$Behavior.advancedmarketplaceProgressBarSettings = function(){ if ($Core.exists(\'#js_advancedmarketplace_form_holder\')) { oProgressBar = {holder: \'#js_advancedmarketplace_form_holder\', progress_id: \'#js_progress_bar\', uploader: \'#js_progress_uploader\', add_more: true, max_upload: ' . (int) Phpfox::getUserParam('advancedmarketplace.total_photo_upload_limit') . ', total: 1, frame_id: \'js_upload_frame\', file_id: \'image[]\'}; $Core.progressBarInit(); } }</script>',
					'pager.css' => 'style_css',
