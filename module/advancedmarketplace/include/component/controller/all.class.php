...
class advancedmarketplace_Component_Controller_All extends Phpfox_Component
{
	...
	public function process()
	{
		...
		if($bIsProfile) {
			// var_dump($bIsProfile === true ? $this->url()->makeUrl($aUser['user_name'], array('advancedmarketplace.search', 'view' => $this->request()->get('view'))) : $this->url()->makeUrl('advancedmarketplace.search.', array('view' => $this->request()->get('view'))));
			// exit;
		}
		$aSearchFields = array(
		...
		if($this->request()->get('sort') == 'recent-viewed')
		{
			$this->url()->send('advancedmarketplace.search.view_'.$this->request()->get('view').'.sort_recent-viewed');
		}
