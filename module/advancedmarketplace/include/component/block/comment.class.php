 ...
 */
class Advancedmarketplace_Component_Block_Comment extends Phpfox_Component 
{
	/**
	...
	public function process()
	{
		...
		{	
			...
			{					
				if ($sFeedType == 'view')
				{
					if (isset($aFeed['comment_display']))
                    {
                        $iPageLimit = $aFeed['comment_display'];
                    }
                    else
                    {
                ...
                $aComments = Phpfox::getService('comment')->getCommentsForFeed($aFeed['comment_type_id'], $aFeed['item_id'], $iPageLimit, $mPager, $iCommentId);
                foreach ($aComments as $i => $aComment)
                {
                    $aComments[$i]['split'] = 15;
                }
                $aFeed['comments'] = $aComments;
			}
		}
		
		if ($sFeedType == 'view' && !isset($aFeed['no_comment_pager']))
		{
