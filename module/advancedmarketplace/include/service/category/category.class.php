...
class AdvancedMarketplace_Service_Category_Category extends Phpfox_Service 
{
	...
	public function get($isForSearch = 0)
	{
		...
		if ($this->_sDisplay == 'admincp')
		{
			...
			{				
				$sOutput = $this->_get(0, 1, $isForSearch);
	...
	private function _get($iParentId, $iActive = null, $isForSearch = 0)
	{
		if ($isForSearch)
        {
            $sName = 'search[category]';
            $sEvent = 'onchange="changeCategory();"';
        }
        else
        {
           $sName = 'val[category][]';
		...
		if (count($aCategories))
		{
			...
			else 
			{
				...
				$this->_sOutput .= '<select ' . $sEvent . ' name="' . $sName . '" class="js_mp_category_list" id="js_mp_id_' . $iParentId . '">' . "\n";
