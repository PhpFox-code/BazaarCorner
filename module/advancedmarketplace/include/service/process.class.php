...
class AdvancedMarketplace_Service_Process extends Phpfox_Service
{
	...
	public function sponsor($iId, $iType)
	{
		...
		Phpfox::isUser(true);
		Phpfox::getUserParam('advancedmarketplace.can_sponsor_advancedmarketplace', true);
		$aListing = Phpfox::getService('advancedmarketplace')->getListing($iId);
		if($aListing["post_status"] == 2) {
...
//                ->select()
    ...
    {
        $aResponse = json_decode(Phpfox::getLib('request')->send('http://maps.google.com/maps/geo?q=' . urlencode($sAddress), array(), 'GET', $_SERVER['HTTP_USER_AGENT']), true);
        $aCoordinates = !empty($aResponse["Placemark"][0]["Point"]["coordinates"]) ? $aResponse["Placemark"][0]["Point"]["coordinates"] : array();
		$sGmapAddress = !empty($aResponse["Placemark"][0]["address"]) ? $aResponse["Placemark"][0]["address"] : '';
